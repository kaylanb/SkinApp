...
def RGB_SkinMap(image_data, alpha0=0.1276, beta0=0.9498, gamma0=2.77):
	...
	map= ((alpha>alpha0)&(beta<=beta0)&(gamma<=gamma0))
	return map.astype('bool')
    
    
def CbCr_SkinMap(image_data, y_min=80, cb_min=77, cb_max=135, cr_min=133, cr_max=185):
...
def CCCM_SkinMap(image):
    ...
    map= np.logical_and(0.026 <= e, e <= .12)
    return map.astype('bool')
...
def percent_skin(skin_image):
    return np.shape(np.where(skin_image))[1]/float(skin_image.size)

def fill_percent(skinMap):
    '''dilates skinMap, fills, subtracts skinMap returns percent pixels remaining
    IDEA: after 2 dilations then fill, skin makes thin outline, non skin makes huge blob'''
    patt = ndimage.generate_binary_structure(2, 1)
    dilated= ndimage.morphology.binary_dilation(skinMap, iterations=2, structure=patt)
    filled = ndimage.morphology.binary_fill_holes(dilated)
    diff=filled-skinMap
    return percent_skin(diff)

#extract SkinLikelihood map at given map value IntVal
